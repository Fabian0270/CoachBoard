...
function fakePayment(p: Partial<Payment>): Payment {
  return {
    ...
    period_start: null, period_end: null, due_date: '2026-06-01',
    paid: 0, paid_at: null, notes: null,
...
describe('paymentStatus helper', () => {
  ...
  it('overdue when unpaid and due date has passed', () => {
    expect(paymentStatus(fakePayment({ paid: 0, due_date: '2026-06-10' }), today)).toBe('overdue')
  })
  it('due_soon when unpaid and due within a week', () => {
    expect(paymentStatus(fakePayment({ paid: 0, due_date: '2026-06-25' }), today)).toBe('due_soon')
  })
  it('upcoming when unpaid and due further out', () => {
    expect(paymentStatus(fakePayment({ paid: 0, due_date: '2026-07-30' }), today)).toBe('upcoming')
  })
  it('paid when settled and coverage well ahead', () => {
    expect(paymentStatus(fakePayment({ paid: 1, period_end: '2026-07-31' }), today)).toBe('paid')
  })
  it('expiring_soon when paid but coverage ends within a week', () => {
    expect(paymentStatus(fakePayment({ paid: 1, period_end: '2026-06-25' }), today)).toBe('expiring_soon')
  })
})

describe('payment CRUD', () => {
  it('creates and lists payments for an athlete', async () => {
    await createPayment({ athlete_id: athleteId, amount: 800, currency: 'SEK', due_date: '2026-06-01' })
  ...
  it('marking paid stamps paid_at; marking unpaid clears it', async () => {
    const p = await createPayment({ athlete_id: athleteId, amount: 800, currency: 'USD', due_date: '2026-06-05' })
  ...
  it('renew clones the next month, unpaid', async () => {
    const p = await createPayment({
      athlete_id: athleteId, amount: 800, currency: 'EUR',
      period_start: '2026-06-01', period_end: '2026-06-30', due_date: '2026-06-01', paid: true,
    })
    const next = await renewPayment(p.id)
    expect(next?.due_date).toBe('2026-07-01')
    expect(next?.period_start).toBe('2026-07-01')
    expect(next?.period_end).toBe('2026-07-30')
  ...
  it('deletes a payment', async () => {
    const p = await createPayment({ athlete_id: athleteId, amount: 1, currency: 'SEK', due_date: '2026-06-01' })
...
describe('getPaymentAlerts', () => {
  it('returns one most-urgent-first alert per athlete for the latest period needing attention', async () => {
    ...
    await createPayment({ athlete_id: a1.id, amount: 800, currency: 'SEK', due_date: '2026-05-01', paid: true, period_end: '2026-05-31' })
    await createPayment({ athlete_id: a1.id, amount: 800, currency: 'SEK', due_date: '2026-06-10' })
    // a2: paid but coverage expiring within the week.
    await createPayment({ athlete_id: a2.id, amount: 50, currency: 'USD', due_date: '2026-06-01', paid: true, period_end: '2026-06-25' })
    // a3: paid and comfortably ahead → no alert.
    await createPayment({ athlete_id: a3.id, amount: 50, currency: 'USD', due_date: '2026-06-01', paid: true, period_end: '2026-12-31' })
